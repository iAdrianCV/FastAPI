...
def authenticate_votante(db, email: str, password: str,):
    votante = get_votante_by_email(db, email)
    if not votante:
        return False
    if not verify_password(password, votante.password):
        return False
    return votante

def create_votante_token(data: dict,tokens_config:TokensConfig, access=True):
    to_encode = data.copy()
    if access:
        ...
        scope = "refresh_token"
    to_encode.update({"user_type": "votante"})
...
async def get_current_votante(token: str = Depends(oauth2_scheme),
    ...
    votante=get_votante_by_email(db,token_data.email)
    if votante is None:
        raise credentials_exception
    return votante
